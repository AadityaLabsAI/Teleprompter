...
export function Teleprompter({ script, wpm, onExit }: TeleprompterProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .2 }} className="teleqen-prompter fixed inset-0 z-50 flex flex-col text-white" onClick={wakeHUD}>
      <div className="pointer-events-none fixed inset-x-0 top-0 z-40 h-[2px] bg-white/5"><div className="prompter-progress h-full transition-[width] duration-150" style={{ width: `${progress}%` }} /></div>
      <div className="pointer-events-none fixed left-5 top-5 z-40 hidden items-center gap-2 sm:flex"><img src="/teleqen-mark.svg" alt="Teleqen" className="prompter-brand h-8 w-8 rounded-[10px] opacity-90" /><span className="text-[10px] font-bold uppercase tracking-[.22em] text-white/30">Teleqen</span></div>
      {showFocusLine && <div className="pointer-events-none fixed inset-x-0 top-[34%] z-20 h-16 -translate-y-1/2"><div className="prompter-focus h-full w-full border-y bg-gradient-to-r from-transparent to-transparent" /></div>}
      <div ref={containerRef} className="prompter-script smooth-scroll-container flex-1 overflow-y-auto px-4 py-[30vh] sm:px-8 lg:px-16 xl:px-24" style={{ transform: isFlipped ? 'scaleX(-1)' : undefined }} aria-label="Teleprompter script">
      ...
      <div className={`teleqen-hud fixed left-1/2 z-50 w-[calc(100%-1rem)] max-w-[calc(100vw-1rem)] -translate-x-1/2 transition-all duration-300 sm:bottom-6 sm:w-auto ${showHUD ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-24 opacity-0'}`}>
        <div className="prompter-glass glass-panel rounded-[1.4rem] p-2 sm:rounded-full sm:p-2.5"><div className="hud-scroll flex max-w-full flex-nowrap items-center justify-start gap-1 overflow-x-auto sm:justify-center sm:gap-2">
          <button onClick={togglePlay} aria-label={isPlaying ? 'Pause scrolling' : 'Start scrolling'} className="prompter-primary flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-white shadow-[0_10px_35px_rgba(139,92,246,.25)] transition duration-200 hover:scale-105 hover:bg-violet-400 active:scale-95 sm:h-16 sm:w-16">{isPlaying ? <Pause className="h-6 w-6 fill-current" /> : <Play className="ml-1 h-6 w-6 fill-current" />}</button>
