...
export function VideoOverlay({ onClose }: { onClose: () => void }) {
  ...
  const onPointerDown = (event: PointerEvent<HTMLDivElement>) => {
    ...
    dragRef.current = { active: true, x: event.clientX, y: event.clientY, left: rect.left, top: rect.top };
  ...
  return (
    <div className={`camera-v2 ${compact ? 'compact' : ''}`} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={() => { dragRef.current.active = false; }} role="dialog" aria-label="Camera preview">
      <div className="camera-v2-bar">
        <div className="camera-v2-title"><span><i /> LIVE</span><b><Camera size={13} /> Camera preview</b></div>
        <div className="camera-v2-tools">
          <button onClick={() => setMirrored((v) => !v)} aria-label="Mirror camera"><FlipHorizontal size={14} /></button>
          <button onClick={() => setCompact((v) => !v)} aria-label="Resize camera">{compact ? <Maximize2 size={14} /> : <Minimize2 size={14} />}</button>
          <button onClick={close} aria-label="Close camera"><X size={14} /></button>
        </div>
      </div>
      <div className="camera-v2-body">
        {error ? <div className="camera-v2-error"><Camera size={22} /><span>{error}</span></div> : <video ref={videoRef} autoPlay muted playsInline style={{ transform: mirrored ? 'scaleX(-1)' : undefined }} />}
      </div>
      <div className="camera-v2-footer">Drag anywhere to reposition</div>
    </div>
  );
}
